SELECT
	...
	e.properties->>'subject_points' AS pts,
	e.properties->>'num_games' 
FROM vertices v JOIN edges e
	ON e.subject_identifier = v.identifier
	AND e.subject_type = v.type
WHERE e.object_type = 'player'::vertex_type AND e.properties->>'subject_points' IS NOT NULL
ORDER BY pts DESC
...
FROM filtered f1
JOIN filtered f2 ON f1.game_id = f2.game_id AND f1.team_id <> f2.team_id
